...
-(id)initWithFrame:(CGRect)frame withPhotoUrl:(NSString *)photoUrl{
    ...
    if (self) {
        ...
        if (!isCached) {//没有缓存
            ...
            [hud addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self.delegate action:@selector(TapHiddenPhotoView)]];
        }
        
        [self.imageView sd_setImageWithURL:[NSURL URLWithString:photoUrl] placeholderImage:[UIImage imageNamed:@"default_portrait_less"] options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize){
