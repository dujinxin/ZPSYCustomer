...
-(void)setCircularFrame:(CGRect)circularFrame{

    if (!CGRectEqualToRect(_circularFrame, circularFrame)) {
        _circularFrame=circularFrame;
    }
...
{
    //验证 裁剪半径是否有效
    self.radius= self.radius > self.frame.size.width/2?self.frame.size.width/2:self.radius;
    
    CGFloat width  = self.frame.size.width;
    CGFloat height = (_image.size.height / _image.size.width) * self.frame.size.width;
    
//    if (height<self.circularFrame.size.height) {
//        height=self.circularFrame.size.height;
//        width  =(_image.size.width / _image.size.height) * self.frame.size.height;
//    }
    ...
    _overView.opaque = NO;
    [_overView setFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height )];
...
-(void)drawClipPath:(ClipType )clipType
{
    ...
    if (CGRectEqualToRect(_circularFrame, CGRectNull)||CGRectEqualToRect(_circularFrame, CGRectZero)) {
        _circularFrame = CGRectMake(center.x - self.radius, center.y - self.radius, self.radius * 2, self.radius * 2);
    }else{

        _circularFrame = CGRectMake(center.x - _circularFrame.size.width/2, center.y - _circularFrame.size.height/2, _circularFrame.size.width, _circularFrame.size.height);
    ...
    {
        //绘制圆形裁剪区域
        [path  appendPath:[UIBezierPath bezierPathWithArcCenter:self.center radius:self.radius startAngle:0 endAngle:2*M_PI clockwise:NO]];
    }
    else
    {
        [path appendPath:[UIBezierPath bezierPathWithRect:_circularFrame]];
    }
...
-(void)MakeImageViewFrameAdaptClipFrame
{
    ...
    if(width < self.circularFrame.size.width)
    {
...
-(void)handlePinGesture:(UIPinchGestureRecognizer *)pinGesture
{
    ...
    {
        pinGesture.scale=1+pinGesture.velocity/50;
